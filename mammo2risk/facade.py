...
class MammoRiskManager(object): 
    ...
    HO_MR_NORMALIZER_CONFIG = {"max_value": 4095, "norm_max_value": 255, "grid_size": 4, "cliplimit": 4}
    def __init__(self, ge_md, ho_md, ge_mr, ho_mr, ge_dr_coef, ge_dr_int, ho_dr_coef, ho_dr_int,
                ge_dr_scaler, ho_dr_scaler, num_features):
      self.GE_MD = ge_md
      self.HO_MD = ho_md
      self.GE_MR = ge_mr
      self.HO_MR = ho_mr
    ...
    def get_density_score_all(self, files): 
      ...
      for i, file in enumerate(files): 
          print(f"{i+1}/{len(files)} Getting density score from {file} ...")
    ...
    def mammo2risk(self, files): 
      ...
      dense_scores = self.get_density_score_all(files)
      deep_mammo_scores = self.get_deep_mammo_risk_all(files)
      ...
      result["denserisk"] = dense_scores
      result["mammorisk"] = deep_mammo_scores 
      return result
